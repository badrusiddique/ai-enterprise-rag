...
A console application that demonstrates **Retrieval-Augmented Generation (RAG)** using a local movie database. Built to explore how to ground LLM responses in a trusted dataset rather than letting the model rely on its own (fallible) training data.

## Why RAG?

Large language models hallucinate. When asked about specific facts — film details, cast, plot — they often produce plausible-sounding but incorrect answers with full confidence.
...
Conversation history is accumulated across turns so follow-up questions remain contextually accurate without re-embedding from scratch.
...
Everything runs locally. No external API calls, no API keys.
...
On first run the app creates the `movies` collection in Qdrant and generates embeddings for all 20 films. Subsequent runs skip this step and query the existing collection directly.
